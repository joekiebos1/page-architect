...
export function HeroBlock({
  ...
  return (
    <>
      <style>{`.hero-background-panel { background-color: #3F007F !important; }`}</style>
      <section
      ...
      >
      {/* Purple panel - ends halfway through image. Parallax: stays at scroll rate. */}
      <div
        className="hero-background-panel"
        style={{
          position: 'absolute',
          top: 0,
          left: 0,
      ...
      {/* Content layer - text, buttons, image. Parallax target: image moves faster on scroll. */}
      <div
        style={{
          position: 'relative',
          zIndex: 1,
      ...
      >
        {/* Text and buttons - purple extends 12.5vw below (half of 2:1 image height) */}
        <div
          style={{
            width: '100%',
            paddingBottom: 'var(--ds-spacing-hero-overlap)',
          }}
        >
        <SurfaceProvider
          level={0}
          hasBoldBackground={true}
          style={{
            width: '100%',
        ...
        >
          ...
          >
            {(productName || headline || subheadline || ctaText || cta2Text) ? (
              <>
                {productName && (
                  <Text size="L" weight="high" color="on-bold-high" align="center" as="span">
                    {productName}
                  </Text>
                )}
                {headline && (
                  <Display size="L" as="h1" color="on-bold-high" align="center" style={{ lineHeight: 1.1 }}>
                    {headline}
                  </Display>
                )}
                {subheadline && (
              ...
              <>
                ...
                <Text size="S" weight="low" color="on-bold-high" align="center" as="p" style={{ margin: 0, maxWidth: 'calc(var(--ds-breakpoint-desktop) / 2 + var(--ds-spacing-l))', lineHeight: 1.4, marginInline: 'auto' }}>
                  With JioHome, your connection is just the beginning. Join us and unlock a world that keeps growing with you with unlimited wi-fi, 1000+ TV channels, 15+ OTT apps, games, and much more.
                </Text>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 'var(--ds-spacing-s)', justifyContent: 'center', marginTop: 'var(--ds-spacing-2xs)' }}>
        ...
        {/* Image - overlaps purple by 12.5vw so purple ends at image midpoint. Parallax target. */}
        <div
          ...
          >
            {hasImage ? (
              <Image
                ...
                alt=""
                fill
                sizes={`(max-width: ${breakpoints.tablet}px) 100vw, ${breakpoints.desktop}px`}
              ...
              >
                <Text
                  ...
                  align="center"
                  as="span"
                  style={{ color: 'var(--ds-color-text-medium)' }}
                >
                  Key visual 16:9
                </Text>
              </div>
